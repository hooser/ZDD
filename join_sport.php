...

{
    ...
    while($row = $res_sport->fetch_assoc()) 
	{
       ...
       $sport_type_id = $row['sport_tid'];
	   $get_kind = "select name from sport_type where  id = $sport_type_id";
	   $res_kind = $conn->query($get_kind);
	   $_kind = $res_kind->fetch_assoc();
	   $sport_kind = $_kind['name'];
	   ...
	   echo     "<td><a href='part_sport.php?id={$row['id']}'><button type='button' id='button' style='cursor: pointer'>detail</button></a></td>";
	   //<a href='try1.php'><input type='button' id='1' name='button' value='点击' onclick="oncli()"></button></a>
       echo     "</tr>";
    }
	echo        "</table>";
} 
